...
class Resource {//重要
    ...
    public synchronized void add() {
        if (num>=MAX_SIZE){
            try {
                System.out.println(Thread.currentThread().getName() +":货满!-->wait before");
                this.wait();
                System.out.println(Thread.currentThread().getName() +":货满!-->wait after");
    ...
    public synchronized void remove() {
        if(num<=0){
            try {
                System.out.println(Thread.currentThread().getName() +":缺货!-->wait before");
                this.wait();
                System.out.println(Thread.currentThread().getName() +":缺货!-->wait after");
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
